...
function EmailAllModal({job,tasks,emailMap,onClose}){
  ...
  const buildBody=()=>{
    const lines=[`Hi team,`,``,`Please see your current open punch list items for ${job.name} below.`,`Your items are listed under your company name.`,``,`─────────────────────────`,``];
    ...
    lines.push(`─────────────────────────`,`View your full punch list online:`,jobUrl,``,`Open the link on any device to see your items, check status, and print or save a PDF.`,``,`Thank you,`,co.name);
    return lines.join("\n");
  };
  ...
  const openOne=(trade)=>{
    ...
    const list=byTrade[trade]||[];
    const subject=`${co.name} — Punch list: ${job.name}`;
    const lines=[`Hi ${trade} team,`,``,`Your current open punch list items for ${job.name}:`,``];
    for(const t of list){
      const overdue=t.dueDate&&t.dueDate<tdy;
      lines.push(`  • ${t.area}: ${t.description}`);
      lines.push(`    Status: ${t.status} | Due: ${fmtDate(t.dueDate)}${overdue?" (PAST DUE)":""}`);
    }
    lines.push(``,`─────────────────────────`,`View punch list online:`,jobUrl,``,`Thank you,`,co.name);
  ...
  return(<Modal onClose={onClose} wide><div style={{padding:20}}>
    ...
    {tradesWithEmail.length>0?(
      <div style={{background:sent?"#EAF2E8":C.mist,border:`1.5px solid ${sent?C.sage:C.line}`,borderRadius:12,padding:"18px 20px",marginBottom:16}}>
        <div style={{fontWeight:700,fontSize:15,color:sent?C.sage:C.ink,marginBottom:4}}>
          {sent?"✓ Email opened in your mail app":"Send one email to all trades"}
        </div>
        <div style={{fontSize:13,color:C.taupe,marginBottom:14,lineHeight:1.6}}>
          Opens your mail app with <b>you in the To field</b> and all <b>{tradesWithEmail.length} trade{tradesWithEmail.length!==1?"s":""} BCC'd</b>.
          Each trade sees only their own address. You get a copy. One click to send.
          {!co.email&&<span style={{color:C.amber,display:"block",marginTop:6}}>⚠ Add your email in ⚙ Settings so you're included in the To field.</span>}
        </div>
        <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
          <Btn onClick={openEmail} style={{fontSize:15,padding:"12px 24px"}}>✉ Open in mail app</Btn>
          <div style={{fontSize:12,color:C.taupe,flex:1,minWidth:0}}>
            To: {co.email||<span style={{color:C.amber}}>(add your email in Settings)</span>}<br/>
