...
function Dashboard({projects,tasks,onOpenJob,onAllJobs,onCalendar,onNewJob,onDirectory,onEmail,onSettings,onStatusChange}){
  ...
  const filtered=jobFilter==="All"?allSorted:allSorted.filter(p=>p.status===(jobFilter==="Archived"?"Archived":jobFilter==="Closed"?"Closed":"Active"));
  const STATUS_OPTIONS=["Active","Closed","Archived"];
  const STATUS_COLORS={Active:C.sage,Closed:C.taupe,Archived:C.amber};
  return(
    <div style={{padding:16,maxWidth:1100,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",gap:0,border:`1px solid ${C.line}`,borderRadius:8,overflow:"hidden",width:"fit-content",marginBottom:14}}>
        {["Active","Closed","Archived","All"].map(f=>(
          <button key={f} onClick={()=>setJobFilter(f)}
            ...
            {f} <span style={{opacity:0.6,fontWeight:400}}>({f==="All"?projects.length:projects.filter(p=>p.status===(f==="Active"?"Active":f)).length})</span>
          </button>
        ))}
      </div>

      {filtered.length===0&&<div style={{padding:40,textAlign:"center",color:C.taupe,background:C.card,borderRadius:12,border:`1px dashed ${C.line}`}}>{jobFilter==="Archived"?"No archived jobs.":"No jobs yet. Create your first job to start a punch list."}</div>}
      <div style={{display:"grid",gap:10,gridTemplateColumns:"repeat(auto-fill,minmax(285px,1fr))"}}>
        {filtered.map(p=>{
          ...
          return(
            <div key={p.id} style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:12,padding:14,opacity:p.status==="Archived"?0.65:1}}>
