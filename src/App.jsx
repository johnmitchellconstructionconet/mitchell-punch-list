  ...
  return(
    ...
    <Shell>
      ...
      {openTask&&<TaskDetail task={openTask} userName={user}
        ...
        trades={trades} projects={projects}
...
function TaskDetail({task,userName,loadPhoto,savePhoto,requestAnnotate,onLightbox,onClose,onUpdate,onDelete,trades,projects}){
  const co=useCompany();
      ...
      {editing&&(
        <div style={{marginTop:14,padding:16,background:C.mist,borderRadius:10,border:`1px solid ${C.line}`}}>
          ...
          <div style={{display:"grid",gap:11,gridTemplateColumns:"1fr 1fr"}}>
            ...
            <div><Lbl>Trade</Lbl>
              <input list="edit-trade-list" value={editF.trade} onChange={e=>setEditF(f=>({...f,trade:e.target.value}))}/>
              <datalist id="edit-trade-list">{(trades||[]).map(t=><option key={t} value={t}/>)}</datalist>
            </div>
...
function Annotator({dataUrl,onCancel,onSave}){
  ...
  return(<div className="no-print" style={{position:"fixed",inset:0,background:"#111",zIndex:80,display:"flex",flexDirection:"column"}}>
    <div style={{padding:"10px 14px",display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",background:"#1a1918"}}>
      ...
      {[3,6,12].map(s=>(<button key={s} onClick={()=>setPen(s)} style={{width:s===3?24:s===6?30:36,height:s===3?24:s===6?30:36,borderRadius:"50%",background:pen===s?"#fff":"#3a3835",border:"none"}}/>))}
    ...
    <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",padding:10,minHeight:0}}>
      <canvas ref={cvRef} onPointerDown={onPD} onPointerMove={onPM} onPointerUp={onPU} onPointerCancel={onPU} style={{maxWidth:"100%",maxHeight:"100%",touchAction:"none",borderRadius:6,background:"#000",cursor:"crosshair"}}/>
    </div>
    <div style={{textAlign:"center",color:"#555",fontSize:12,paddingBottom:10}}>Draw freely to circle or mark up the issue. Choose color and weight above.</div>
  </div>);
}
