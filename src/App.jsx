        ...
        {filtered.map(p=>{
          ...
          return(
            <div key={p.id} style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:12,overflow:"hidden",opacity:p.status==="Closed"?0.7:1}}>
              {/* Header photo */}
              <JobCardPhoto project={p} loadPhoto={loadPhoto} onOpen={()=>onOpenJob(p.name)}/>
...
function JobCardPhoto({project,loadPhoto,onOpen}){
  const [src,setSrc]=useState(null);
  useEffect(()=>{
    if(project?.headerPhotoId&&loadPhoto) loadPhoto(project.headerPhotoId).then(v=>v&&setSrc(v));
    else setSrc(null);
  ...
  if(!src) return null;
  return(
    <div onClick={onOpen} style={{width:"100%",height:140,overflow:"hidden",position:"relative",cursor:"pointer"}}>
      <img src={src} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
...
function TaskCard({task,showProject,onOpen,loadPhoto}){
  ...
  return(
    <div onClick={onOpen} style={{background:C.card,border:`1px solid ${C.line}`,borderLeft:`5px solid ${accent}`,borderRadius:10,cursor:"pointer",overflow:"hidden"}}>
      ...
      <div style={{padding:"10px 13px 9px",borderBottom:`1px solid ${C.line}`,display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,background:C.mist}}>
        <div style={{fontSize:12,fontWeight:700,color:C.taupe}}>
          {showProject&&<span>{task.project} · </span>}
          <span style={{color:C.ink}}>{task.area}</span>
        </div>
      ...
      <div style={{padding:"10px 13px",borderBottom:`1px solid ${C.line}`}}>
        <div style={{fontWeight:700,fontSize:14,lineHeight:1.4,color:C.ink}}>{task.description}</div>
      </div>

      {/* Section 3 — Key details */}
      <div style={{padding:"9px 13px",borderBottom:(task.photos||[]).length>0||((task.comments||[]).length>0)?`1px solid ${C.line}`:"none",display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5px 12px",fontSize:12.5}}>
        <div><span style={{...CAPT,fontSize:9.5,color:C.stone,display:"block",marginBottom:1}}>Assignee</span><span style={{fontWeight:600,color:C.ink}}>{task.trade||"—"}</span></div>
        <div><span style={{...CAPT,fontSize:9.5,color:C.stone,display:"block",marginBottom:1}}>Priority</span><span style={{fontWeight:700,color:PRI_FG[task.priority]}}>{task.priority}</span></div>
        <div><span style={{...CAPT,fontSize:9.5,color:C.stone,display:"block",marginBottom:1}}>Due date</span><span style={{fontWeight:600,color:overdue?C.rust:C.ink}}>{fmtDate(task.dueDate)}{overdue?" ⚠":""}</span></div>
        <div><span style={{...CAPT,fontSize:9.5,color:C.stone,display:"block",marginBottom:1}}>Reported by</span><span style={{fontWeight:600,color:C.ink}}>{task.createdBy||"—"}</span></div>
      </div>

      {/* Section 4 — Photos + comments (only if present) */}
      {((task.photos||[]).length>0||(task.comments||[]).length>0)&&(
        <div style={{padding:"8px 13px",display:"flex",alignItems:"center",gap:8}}>
          ...
          {(task.comments||[]).length>0&&(
            <div style={{fontSize:12,color:C.taupe,whiteSpace:"nowrap"}}>💬 {task.comments.length}</div>
          )}
        </div>
      )}
    </div>
      ...
      <div style={{padding:0,overflow:"hidden",borderRadius:14}}>
        ...
        <Section title="Task Details">
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px 16px"}}>
            ...
            <div><div style={{...CAPT,fontSize:9.5,color:C.stone,marginBottom:3}}>Reported on</div><div style={{fontWeight:700}}>{fmtDate(new Date(task.createdAt).toISOString())}</div></div>
          </div>
          {task.approval==="Approved"&&(
            <div style={{marginTop:12,padding:"9px 12px",background:"#EAF2E8",border:`1.5px solid ${C.sage}`,borderRadius:8,color:C.sage,fontWeight:700,fontSize:13}}>
              ✓ Approved by {task.approvedBy} — {fmtDate(new Date(task.approvedAt).toISOString())}
            </div>
          )}
        </Section>
...
<script>window.onload=function(){setTimeout(function(){window.print()},400)}<\/script>
    ...
    setPosterMsg("Downloaded! Open the file — print dialog will open automatically. Choose 'Save as PDF'.");
...
function Report({tasks,jobLabel,filters,userName,loadPhoto,onBack,project}){
  ...
  const doExport=async()=>{setExporting(true);try{
    ...
    // Pre-load header photo so it's available as data URL in the exported HTML
    let headerDataUrl=null;
    if(project?.headerPhotoId) headerDataUrl=await loadPhoto(project.headerPhotoId);
    if(!headerDataUrl&&headerSrc) headerDataUrl=headerSrc;
    const sm=s=>{const m=STATUS_META[s]||STATUS_META.Reported;return`background:${m.bg};color:${m.fg};padding:2px 8px;border-radius:5px;font-weight:700;font-size:11px;text-transform:uppercase`;};
