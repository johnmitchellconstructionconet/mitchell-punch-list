...
function JobLinkApp({ jobId }) {
  ...
  const load = useCallback(async () => {
    setSyncing(true); setLoadError(false);
    try {
      const timeout = ms => new Promise((_,r)=>setTimeout(()=>r(new Error("timeout")),ms));
      const [projs, allTasks, csRaw] = await Promise.race([
        Promise.all([getProjects(), getTasks(), getSetting("cosettings")]),
        timeout(20000),
      ]);
  ...
  if (!loaded) return (
    <Shell>
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,background:"#F8F8F7",padding:24}}>
        {loadError ? (<>
          <div style={{fontSize:28}}>⚠️</div>
          <div style={{fontFamily:"Raleway,sans-serif",fontSize:16,fontWeight:600,color:"#1E2336"}}>Could not load job</div>
          <div style={{fontSize:13,color:"#9B8E7E",textAlign:"center",maxWidth:300,lineHeight:1.6}}>Database may be waking up. Wait 10 seconds and try again.</div>
          <button onClick={load} style={{background:"#1E2336",color:"#fff",border:"none",borderRadius:8,padding:"11px 28px",fontSize:14,fontWeight:600,cursor:"pointer"}}>↻ Try again</button>
        </>) : (
          <Loader txt="Loading job…"/>
        )}
      </div>
    </Shell>
  );
...
function InternalApp() {
  ...
  const loadAll = useCallback(async()=>{
    ...
    try {
      const timeout = ms => new Promise((_,r)=>setTimeout(()=>r(new Error("timeout")),ms));
      const [p, t, c, tc, cs] = await Promise.race([
        Promise.all([
      ...
      setProjects(p); setTasks(t); setCompanies(c);
      setTeamCode(tc||"");
  ...
  const loadCode = async () => teamCode;  // already loaded by loadAll — no extra round trip

  if(!authed) return <Shell><InternalLogin loadCode={loadCode} onAuth={name=>{setUser(name);setAuthed(true);}}/></Shell>;
  if(!loaded) return (
    <Shell>
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,background:"#F0F1F5",padding:24}}>
        {loadError ? (<>
          <div style={{fontSize:32}}>⚠️</div>
          <div style={{fontFamily:"Raleway,sans-serif",fontSize:18,fontWeight:600,color:"#1E2336"}}>Could not connect to database</div>
          <div style={{fontSize:14,color:"#9B8E7E",textAlign:"center",maxWidth:360,lineHeight:1.6}}>
            Supabase may be waking up (free tier pauses after inactivity). Wait 10 seconds and try again.
          </div>
          <button onClick={loadAll} style={{background:"#1E2336",color:"#fff",border:"none",borderRadius:10,padding:"13px 32px",fontSize:15,fontWeight:600,cursor:"pointer",marginTop:4}}>
            ↻ Try again
          </button>
          <div style={{fontSize:12,color:"#B5AFA7",marginTop:8}}>
            If this keeps failing, check your Supabase project is not paused at supabase.com
          </div>
        </>) : (<>
          <div style={{fontFamily:"Raleway,sans-serif",fontSize:15,color:"#9B8E7E"}}>Connecting to database…</div>
          <div style={{width:200,height:3,background:"#E4E2DD",borderRadius:2,overflow:"hidden"}}>
            <div style={{width:"50%",height:"100%",background:"#1E2336",borderRadius:2,animation:"sl 1.4s ease-in-out infinite"}}/>
          </div>
