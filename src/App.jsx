...
<script>window.onload=function(){setTimeout(function(){window.print()},400)}<\/script>
    ...
    a.download=`punch-list-${project.name.replace(/[^a-z0-9]+/gi,"-").toLowerCase()}-${tdy2}.html`;
    ...
    <CompanyCtx.Provider value={coSettings}>
      <Shell>
        <div style={{background:C.card,borderBottom:`1px solid ${C.line}`,padding:"12px 18px",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
          <Wordmark size={28}/>
          <div style={{flex:1}}/>
          <button onClick={exportList} style={{background:coSettings.accentColor||C.gold,color:"#2E2B28",border:"none",padding:"8px 14px",fontSize:13,borderRadius:7,cursor:"pointer",fontWeight:600}}>⬇ Print / Export PDF</button>
        ...
        <div style={{background:C.mist,borderBottom:`1px solid ${C.line}`}}>
          {/* Header photo */}
          <PubHeaderPhoto project={project} loadPhoto={loadPhoto}/>
...
function PubHeaderPhoto({project,loadPhoto}){
  const [src,setSrc]=useState(null);
  useEffect(()=>{
    if(project?.headerPhotoId&&loadPhoto) loadPhoto(project.headerPhotoId).then(v=>v&&setSrc(v));
  },[project?.headerPhotoId,loadPhoto]);
  if(!src) return null;
  return(
    <div style={{width:"100%",height:220,overflow:"hidden",position:"relative"}}>
      ...
      <div style={{position:"absolute",bottom:14,left:18,color:"#fff"}}>
        <div style={{...DISP,fontSize:26,fontWeight:600,textShadow:"0 1px 4px rgba(0,0,0,0.5)"}}>{project.name}</div>
        {(project.client||project.address)&&<div style={{fontSize:13,opacity:0.85}}>{[project.client,project.address].filter(Boolean).join(" · ")}</div>}
      </div>
    </div>
  );
}
    ...
    <Modal onClose={onClose} wide>
      <div style={{padding:0,overflow:"hidden",borderRadius:14}}>
        ...
        <div style={{padding:"16px 18px"}}>
          ...
          {/* Direct link to this task */}
          <div style={{padding:"12px 14px",background:C.mist,borderRadius:9,borderLeft:`3px solid ${accent}`}}>
            <div style={{...CAPT,fontSize:10,fontWeight:700,color:C.taupe,marginBottom:4}}>Direct link to this job's punch list</div>
            <div style={{fontSize:11.5,fontFamily:"monospace",wordBreak:"break-all",color:C.ink,marginBottom:8}}>{jobUrl}</div>
            <button onClick={()=>{try{navigator.clipboard.writeText(jobUrl);window.alert("Link copied!");}catch{window.alert(jobUrl);}}}
              style={{background:accent,color:"#2E2B28",border:"none",borderRadius:7,padding:"7px 14px",fontSize:13,fontWeight:600,cursor:"pointer"}}>
              📋 Copy link
            </button>
          </div>
...
function InternalApp() {
  ...
  return(
    ...
    <Shell>
      ...
      {showNewJob&&<NewJobModal savePhoto={savePhoto} onCancel={()=>setShowNewJob(false)} onCreate={async j=>{await addProject(j);setShowNewJob(false);setCurrentJob(j.name);}}/>}
      {showEditJob&&currProj&&<EditJobModal project={currProj} savePhoto={savePhoto} loadPhoto={loadPhoto} onCancel={()=>setShowEditJob(false)} onSave={async j=>{await updateProjectState(j);setShowEditJob(false);}}/>}
...
function Dashboard({projects,tasks,onOpenJob,onAllJobs,onCalendar,onNewJob,onDirectory,onEmail,onSettings,onStatusChange}){
  const co=useCompany();
    ...
    <div style={{padding:16,maxWidth:1100,margin:"0 auto"}}>
      ...
      <div style={{display:"grid",gap:10,gridTemplateColumns:"repeat(auto-fill,minmax(285px,1fr))"}}>
        {filtered.map(p=>{
          ...
          return(
            <div key={p.id} style={{background:C.card,border:`1px solid ${C.line}`,borderRadius:12,padding:14,opacity:p.status==="Closed"?0.7:1}}>
...
function JobBar({project,jobLabel,onBack,onQR,onEmailAll,onEdit,loadPhoto}){
  const [headerSrc,setHeaderSrc]=useState(null);
  useEffect(()=>{
    if(project?.headerPhotoId&&loadPhoto) loadPhoto(project.headerPhotoId).then(v=>v&&setHeaderSrc(v));
    else setHeaderSrc(null);
  },[project?.headerPhotoId,loadPhoto]);
    ...
    <div style={{background:C.card,borderBottom:`1px solid ${C.line}`}}>
      {headerSrc&&(
        <div style={{width:"100%",height:180,overflow:"hidden",position:"relative"}}>
          <img src={headerSrc} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
          ...
          <div style={{position:"absolute",bottom:12,left:16,color:"#fff"}}>
            <div style={{...DISP,fontSize:22,fontWeight:600,lineHeight:1.1,textShadow:"0 1px 4px rgba(0,0,0,0.5)"}}>{jobLabel}</div>
            {project&&<div style={{fontSize:12,opacity:0.85}}>{[project.client,project.address].filter(Boolean).join(" · ")}</div>}
          </div>
        </div>
      )}
...
function TaskCard({task,showProject,onOpen,loadPhoto}){
  ...
  return(
    <div onClick={onOpen} style={{background:C.card,border:`1px solid ${C.line}`,borderLeft:`5px solid ${STATUS_META[task.status]?.fg||C.taupe}`,borderRadius:10,padding:14,cursor:"pointer"}}>
      {/* Top row: location + chips */}
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:6}}>
        <div style={{fontSize:12,fontWeight:600,color:C.taupe}}>
          {showProject&&<span>{task.project} · </span>}<span style={{color:C.ink}}>{task.area}</span>
        </div>
      ...
      {/* Description */}
      <div style={{fontWeight:600,fontSize:14,lineHeight:1.4,marginBottom:8}}>{task.description}</div>
      {/* Detail grid */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"3px 12px",fontSize:12.5,marginBottom:8}}>
        <div style={{color:C.taupe}}><span style={{fontWeight:600,color:C.ink}}>Trade: </span>{task.trade||"—"}</div>
        <div style={{color:PRI_FG[task.priority],fontWeight:600}}>{task.priority} priority</div>
        <div style={{color:overdue?C.rust:C.taupe,fontWeight:overdue?700:400}}>
          <span style={{fontWeight:600,color:overdue?C.rust:C.ink}}>Due: </span>{fmtDate(task.dueDate)}{overdue?" · OVERDUE":""}
        </div>
        <div style={{color:C.taupe}}><span style={{fontWeight:600,color:C.ink}}>By: </span>{task.createdBy||"—"}</div>
      </div>
      {/* Photo strip */}
      {(task.photos||[]).length>0&&(
        <div style={{display:"flex",gap:5,marginBottom:8}}>
          {(task.photos||[]).slice(0,4).map(pid=><PhotoThumb key={pid} pid={pid} loadPhoto={loadPhoto} size={56}/>)}
          {(task.photos||[]).length>4&&<div style={{width:56,height:56,borderRadius:7,background:C.mist,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:C.taupe,fontWeight:600}}>+{task.photos.length-4}</div>}
        </div>
      )}
      {/* Comments count */}
      {(task.comments||[]).length>0&&(
        <div style={{fontSize:12,color:C.taupe}}>💬 {task.comments.length} comment{task.comments.length!==1?"s":""}</div>
      )}
    </div>
  );
}
...
function TaskDetail({task,userName,loadPhoto,savePhoto,requestAnnotate,onLightbox,onClose,onUpdate,onDelete,trades,projects,companies}){
  ...
  return(
    <Modal onClose={onClose} wide><div style={{padding:18}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:12.5,fontWeight:600,color:C.taupe}}>{task.project} · {task.area} · {task.trade}
            {companies?.find(c=>c.name===task.trade)&&(()=>{
              const co=companies.find(c=>c.name===task.trade);
              return <span style={{fontWeight:400}}>{co.contactName?" · "+co.contactName:""}{co.phone?" · "+co.phone:""}</span>;
            })()}
          </div>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center",flexShrink:0}}>
          <Btn kind="ghost" onClick={()=>{setEditF({area:task.area,description:task.description,trade:task.trade,priority:task.priority,dueDate:task.dueDate||""});setEditing(true);}} style={{fontSize:13,padding:"8px 12px"}}>✏ Edit</Btn>
      ...
      {/* Inline edit form */}
      {editing&&(
        <div style={{marginTop:14,padding:16,background:C.mist,borderRadius:10,border:`1px solid ${C.line}`}}>
          <div style={{...CAPT,fontSize:11,fontWeight:700,color:C.taupe,marginBottom:12}}>Edit task</div>
          <div style={{display:"grid",gap:11,gridTemplateColumns:"1fr 1fr"}}>
            ...
            <div><Lbl>Trade</Lbl>
              <select value={editF.trade} onChange={e=>setEditF(f=>({...f,trade:e.target.value}))}>
                ...
                {(trades||[]).filter(t=>companies?.find(c=>c.name===t)).length>0&&
                  <optgroup label="Trade Directory">
                    {(trades||[]).filter(t=>companies?.find(c=>c.name===t)).map(t=><option key={t} value={t}>{t}</option>)}
                  </optgroup>}
                {(trades||[]).filter(t=>!companies?.find(c=>c.name===t)).length>0&&
            ...
            <div style={{gridColumn:"1/-1"}}><Lbl>Description</Lbl><textarea rows={3} value={editF.description} onChange={e=>setEditF(f=>({...f,description:e.target.value}))}/></div>
            <div><Lbl>Priority</Lbl>
              <select value={editF.priority} onChange={e=>setEditF(f=>({...f,priority:e.target.value}))}>
      ...
      {tab==="detail"&&(<div style={{paddingTop:14}}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
          ...
          <Btn kind="green" onClick={()=>onUpdate({approval:"Approved",status:"Done",approvedBy:userName,approvedAt:Date.now(),statusHistory:[...(task.statusHistory||[]),{status:"Approved",by:userName,ts:Date.now()}]})}>✓ Approve</Btn>
          ...
          {task.approval==="Approved"&&<Btn kind="ghost" onClick={()=>onUpdate({approval:"Pending",status:"Reported",approvedBy:null,approvedAt:null})}>Clear approval</Btn>}
          <div style={{flex:1}}/>
...
function NewJobModal({onCancel,onCreate,savePhoto}){
  ...
  const handlePhoto=async e=>{const file=e.target.files[0];e.target.value="";if(!file)return;const c=await compress(file,1600,0.8);setHeaderPhoto(c);};
  return(<Modal onClose={onCancel}><div style={{padding:18}}>
    ...
    <div style={{marginBottom:14}}>
      <Lbl>Header photo (optional)</Lbl>
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        {headerPhoto&&<img src={headerPhoto} alt="" style={{width:120,height:70,objectFit:"cover",borderRadius:8,border:`1px solid ${C.line}`}}/>}
        <Btn kind="ghost" onClick={()=>fileRef.current.click()} style={{fontSize:13}}>📷 {headerPhoto?"Change photo":"Add photo"}</Btn>
        {headerPhoto&&<button onClick={()=>setHeaderPhoto(null)} style={{fontSize:12,color:C.rust,background:"transparent",border:"none",cursor:"pointer"}}>Remove</button>}
        <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handlePhoto} style={{display:"none"}}/>
      </div>
    </div>
    <div style={{display:"flex",gap:9,justifyContent:"flex-end"}}><Btn kind="ghost" onClick={onCancel}>Cancel</Btn><Btn disabled={!ok} style={{opacity:ok?1:0.4}} onClick={async()=>{let hpid=null;if(headerPhoto&&savePhoto)hpid=await savePhoto(headerPhoto);onCreate({id:uid(),name:f.name.trim(),client:f.client.trim(),address:f.address.trim(),siteContact:f.siteContact.trim(),sitePhone:f.sitePhone.trim(),status:"Active",headerPhotoId:hpid,createdAt:Date.now()});}}>Create</Btn></div>
  </div></Modal>);
}

function EditJobModal({project,onCancel,onSave,savePhoto,loadPhoto}){
  ...
  const [headerPhoto,setHeaderPhoto]=useState(null);
  const [existingSrc,setExistingSrc]=useState(null);
  const fileRef=useRef();
  const set=k=>e=>setF({...f,[k]:e.target.value}); const ok=f.name.trim();

  useEffect(()=>{
    if(project.headerPhotoId&&loadPhoto){loadPhoto(project.headerPhotoId).then(v=>v&&setExistingSrc(v));}
  },[project.headerPhotoId,loadPhoto]);

  const handlePhoto=async e=>{const file=e.target.files[0];e.target.value="";if(!file)return;const c=await compress(file,1600,0.8);setHeaderPhoto(c);};
  const preview=headerPhoto||existingSrc;
  ...
  return(<Modal onClose={onCancel}><div style={{padding:18}}>
    ...
    <div style={{marginBottom:14}}>
      <Lbl>Header photo</Lbl>
      <div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
        {preview&&<img src={preview} alt="" style={{width:120,height:70,objectFit:"cover",borderRadius:8,border:`1px solid ${C.line}`}}/>}
        <Btn kind="ghost" onClick={()=>fileRef.current.click()} style={{fontSize:13}}>📷 {preview?"Change photo":"Add photo"}</Btn>
        {preview&&<button onClick={()=>{setHeaderPhoto(null);setExistingSrc(null);}} style={{fontSize:12,color:C.rust,background:"transparent",border:"none",cursor:"pointer"}}>Remove</button>}
        <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handlePhoto} style={{display:"none"}}/>
      </div>
    </div>
    <div style={{display:"flex",gap:9,justifyContent:"flex-end"}}>
      <Btn kind="ghost" onClick={onCancel}>Cancel</Btn>
      <Btn disabled={!ok} style={{opacity:ok?1:0.4}} onClick={async()=>{
        let hpid=project.headerPhotoId||null;
        if(headerPhoto&&savePhoto) hpid=await savePhoto(headerPhoto);
        if(!preview) hpid=null;
        onSave({...project,name:f.name.trim(),client:f.client.trim(),address:f.address.trim(),siteContact:f.siteContact.trim(),sitePhone:f.sitePhone.trim(),headerPhotoId:hpid});
      }}>Save changes</Btn>
    </div>
  </div></Modal>);
}
...
function EmailAllModal({job,tasks,emailMap,onClose}){
  const co=useCompany();
  const [sent,setSent]=useState({});
  const [sending,setSending]=useState(false);
  const tdy=today();

  // Job-specific public link
  const jobUrl=(()=>{try{return window.location.href.split("?")[0]+"?job="+job.id;}catch{return "";}})();

  // Group open tasks by trade
  const byTrade={};
  ...
  const bodyFor=(trade,list)=>{
    const lines=[
      `Hi ${trade} team,`,``,
      `Here is your current punch list for ${job.name}:`,``,
    ];
    for(const t of list){
      const overdue=t.dueDate&&t.dueDate<tdy;
      lines.push(`• ${t.area}: ${t.description}`);
      lines.push(`  Status: ${t.status} | Due: ${fmtDate(t.dueDate)}${overdue?" (PAST DUE)":""}`);
    }
    lines.push(
      ``,
      `─────────────────────────`,
      `View & print your full punch list online:`,
      jobUrl,
      ``,
      `Open the link on any device to see your items, check status, and print or save a PDF.`,
      ``,
      `Thank you,`,
      co.name
    );
  ...
  const sendOne=(trade)=>{
    ...
    document.body.appendChild(a);a.click();a.remove();
    try{navigator.clipboard.writeText(`To: ${to}\nSubject: ${subject}\n\n${body}`);}catch{}
    setSent(s=>({...s,[trade]:true}));
  };

  const sendAll=async()=>{
    setSending(true);
    for(const trade of tradesWithEmail){
      sendOne(trade);
  ...
  const allSent=tradesWithEmail.length>0&&tradesWithEmail.every(t=>sent[t]);
  ...
  return(<Modal onClose={onClose} wide><div style={{padding:20}}>
    ...
    <p style={{fontSize:13.5,color:C.taupe,margin:"10px 0 16px"}}>
      Sends each trade their open punch list items for this job. One email per company, pre-written and ready to send.
    </p>

    {/* Summary stats */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:18}}>
      <div style={{background:C.mist,borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
        <div style={{...DISP,fontSize:28,fontWeight:600,color:C.ink}}>{trades.length}</div>
        <div style={{...CAPT,fontSize:10,color:C.taupe,marginTop:3}}>Trades with items</div>
      </div>
      <div style={{background:C.mist,borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
        <div style={{...DISP,fontSize:28,fontWeight:600,color:C.sage}}>{tradesWithEmail.length}</div>
        <div style={{...CAPT,fontSize:10,color:C.taupe,marginTop:3}}>Can receive email</div>
      </div>
      <div style={{background:C.mist,borderRadius:10,padding:"12px 14px",textAlign:"center"}}>
        <div style={{...DISP,fontSize:28,fontWeight:600,color:tasks.length>0?C.ink:C.taupe}}>{tasks.length}</div>
        <div style={{...CAPT,fontSize:10,color:C.taupe,marginTop:3}}>Open items total</div>
      </div>
    </div>

    {/* Send all button */}
    {tradesWithEmail.length>0&&(
      <div style={{marginBottom:16,padding:"14px 16px",background:allSent?"#EAF2E8":C.mist,borderRadius:10,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
        <div style={{flex:1,fontSize:14,fontWeight:600,color:allSent?C.sage:C.ink}}>
          {allSent?"✓ All trades notified":"Send punch list to all "+tradesWithEmail.length+" trade"+(tradesWithEmail.length!==1?"s":"")}
        </div>
        {!allSent&&<Btn onClick={sendAll} disabled={sending} style={{opacity:sending?0.6:1}}>
          {sending?"Opening emails…":"✉ Send all now"}
        </Btn>}
      </div>
    )}
    ...
    <div style={{display:"grid",gap:8}}>
      {trades.map(trade=>{
        ...
        return(
          <div key={trade} style={{background:"#fff",border:`1px solid ${C.line}`,borderRadius:10,padding:"11px 14px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",opacity:!email?0.6:1}}>
            <div style={{flex:"1 1 180px",minWidth:0}}>
              <div style={{fontWeight:700,display:"flex",alignItems:"center",gap:8}}>
                {trade}
                {sent[trade]&&<span style={{...CAPT,fontSize:10,color:C.sage,fontWeight:700}}>✓ sent</span>}
              </div>
              <div style={{fontSize:13,color:C.taupe,marginTop:2}}>
                {list.length} item{list.length!==1?"s":""}
                {overdue>0&&<span style={{color:C.rust,fontWeight:700}}> · {overdue} overdue</span>}
                {" · "}{email||<span style={{color:C.rust}}>no email — add in Trade Directory</span>}
              </div>
            </div>
            {email&&<Btn kind="ghost" onClick={()=>sendOne(trade)} style={{fontSize:13,padding:"8px 14px"}}>
              {sent[trade]?"✓ Sent":"✉ Send"}
            </Btn>}
          </div>
        );
      })}
    ...
    {tradesNoEmail.length>0&&(
      <div style={{marginTop:12,fontSize:13,color:C.taupe,padding:"10px 14px",background:C.mist,borderRadius:8}}>
        <b>{tradesNoEmail.length} trade{tradesNoEmail.length!==1?"s":""}</b> have no email on file and won't receive notifications: {tradesNoEmail.join(", ")}. Add emails in Trade Directory.
      </div>
    )}
...
function Report({tasks,jobLabel,filters,userName,loadPhoto,onBack,project}){
  ...
  const doExport=async()=>{setExporting(true);try{
    ...
    const headerPhotoHtml=headerSrc?`<div style="width:100%;height:200px;margin-bottom:0;overflow:hidden;position:relative;border-radius:8px 8px 0 0"><img src="${headerSrc}" style="width:100%;height:100%;object-fit:cover"><div style="position:absolute;inset:0;background:linear-gradient(to bottom,transparent 40%,rgba(0,0,0,0.6))"></div><div style="position:absolute;bottom:12px;left:16px;color:#fff;font-family:'Cormorant Garamond',Georgia,serif;font-size:24px;font-weight:600">${esc(jobLabel)}</div></div>`:"";
    const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Punch List — ${esc(jobLabel)}</title><style>@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@500;600;700&family=Raleway:wght@300;400;500;600;700&display=swap');body{font-family:Raleway,sans-serif;color:#2E2B28;max-width:860px;margin:0 auto;padding:${headerSrc?"0":"32px 24px"}}.content{padding:${headerSrc?"24px 24px 32px":"0"}}.hdr{display:flex;justify-content:space-between;border-bottom:3px solid #2E2B28;padding-bottom:12px}h1{font-family:'Cormorant Garamond',Georgia,serif;font-size:30px;font-weight:600;margin:6px 0 2px}h2{font-family:'Cormorant Garamond',Georgia,serif;font-size:20px;font-weight:600;border-bottom:2px solid #DDD9D3;padding-bottom:3px;margin:24px 0 5px}h3{font-size:11px;text-transform:uppercase;letter-spacing:0.1em;color:#7B756E;margin:12px 0 3px}table{width:100%;border-collapse:collapse;font-size:13px}th{text-align:left;color:#7B756E;border-bottom:1px solid #B2A98B;padding:5px 6px;font-size:11px;text-transform:uppercase}td{border-bottom:1px solid #E8E4DE;padding:5px 6px;vertical-align:top}.meta{text-align:right;font-size:12px;color:#555}.foot{margin-top:36px;font-size:11px;color:#9A9590;border-top:1px solid #DDD9D3;padding-top:8px}@media print{body{padding:0}.content{padding:${headerSrc?"16px":"0"}}}</style></head><body>${headerPhotoHtml}<div class="content"><div class="hdr"><div>${logoHtml}<h1>Punch List Report</h1><div style="font-size:12px;letter-spacing:0.08em;text-transform:uppercase;font-weight:600;color:#7B756E;margin-top:3px">${esc(jobLabel)}</div>${project&&project.siteContact?`<div style="font-size:12px;color:#8A8279;margin-top:3px">Site: ${esc(project.siteContact)}${project.sitePhone?" · "+esc(project.sitePhone):""}</div>`:""}</div><div class="meta"><div>Generated ${fmtDate(today())} by ${esc(userName)}</div><div style="font-weight:700;margin-top:3px">${open} open · ${done} approved · ${tasks.length} total</div></div></div>${body}<div class="foot">${esc(co.name)}${[co.address,co.city,co.state].filter(Boolean).length>0?" · "+[co.address,co.city,co.state].filter(Boolean).map(esc).join(", "):""}${co.phone?" · "+esc(co.phone):""}<div style="font-family:'Cormorant Garamond',Georgia,serif;font-style:italic;color:${co.accentColor||"#BBA270"};font-size:13px;margin-top:2px">"${esc(co.tagline||"")}"</div></div></div><script>window.onload=function(){setTimeout(function(){window.print()},400)}<\/script></body></html>`;
  ...
  return(<div style={{background:"#fff",minHeight:"100vh"}}>
    ...
    {/* Header photo in preview */}
    {headerSrc&&(
      <div style={{width:"100%",height:200,overflow:"hidden",position:"relative"}}>
        <img src={headerSrc} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
        <div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 40%,rgba(0,0,0,0.6))"}}/>
        <div style={{position:"absolute",bottom:14,left:24,color:"#fff",fontFamily:"'Cormorant Garamond',Georgia,serif",fontSize:26,fontWeight:600}}>{jobLabel}</div>
      </div>
    )}
