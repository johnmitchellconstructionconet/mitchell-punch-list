    ...
    <Modal onClose={onClose} wide>
      <div style={{padding:0,overflow:"hidden",borderRadius:14}}>
        ...
        <div style={{padding:"16px 18px",background:isRejected?"#FDF0EF":STATUS_META[task.status]?.bg||C.mist,borderBottom:`1px solid ${C.line}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
            ...
            <div style={{display:"flex",gap:7,flexShrink:0}}>
              <Btn kind="ghost" onClick={()=>{setEditF({area:task.area,description:task.description,trade:task.trade,priority:task.priority,dueDate:task.dueDate||""});setEditing(!editing);}} style={{fontSize:13,padding:"8px 12px"}}>{editing?"✕ Cancel":"✏ Edit"}</Btn>
        ...
        <Section title="Progress & Approval">
          ...
          {showRejectForm&&(
            <div style={{marginTop:12,padding:"14px 16px",background:"#FDF0EF",border:`1.5px solid ${C.rust}`,borderRadius:10}}>
              <div style={{...CAPT,fontSize:10,fontWeight:700,color:C.rust,marginBottom:8}}>Rejection reason — be specific about what needs to be fixed</div>
              <textarea rows={3} value={rejectReason} onChange={e=>setRejectReason(e.target.value)}
                placeholder="e.g. Grout lines uneven in NW corner, tile lippage exceeds 1/16&quot; — needs to be re-done…"
                style={{width:"100%",marginBottom:10,border:`1px solid ${C.rust}50`,borderRadius:8,padding:"10px 12px",fontSize:14,resize:"vertical",boxSizing:"border-box"}}/>
              <div style={{display:"flex",gap:8,justifyContent:"flex-end"}}>
                <Btn kind="ghost" onClick={()=>{setShowRejectForm(false);setRejectReason("");}}>Cancel</Btn>
                <Btn kind="red" disabled={!rejectReason.trim()} style={{opacity:rejectReason.trim()?1:0.4}} onClick={handleReject}>Confirm rejection</Btn>
              </div>
            </div>
          )}
        </Section>
