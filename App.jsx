        ...
        <div style={{background:C.mist,borderBottom:`1px solid ${C.line}`,padding:"14px 18px"}}>
          ...
          {trades.length>1&&(
            <div style={{marginTop:12}}>
              <div style={{...CAPT,fontSize:10,fontWeight:700,color:C.stone,marginBottom:7}}>Filter by trade</div>
              <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                {["All",...trades].map(t=>(
                  <button key={t} onClick={()=>setTradeFilter(t)}
                    style={{padding:"8px 14px",fontSize:13,fontWeight:600,borderRadius:20,border:`1.5px solid ${tradeFilter===t?(coSettings.accentColor||C.gold):C.line}`,background:tradeFilter===t?(coSettings.accentColor||C.gold):"#fff",color:tradeFilter===t?"#2E2B28":C.ink,cursor:"pointer"}}>
                    {t}
                  </button>
                ))}
              </div>
